...
class WC_CP_Order {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_printable_order_receipt_css', array( $this, 'add_composited_item_order_receipt_styles' ), 10 );
		add_filter( 'woocommerce_printable_order_receipt_line_item_display_data', array( $this, 'add_composited_item_order_receipt_data' ), 10, 2 );
	}
	...
	*/
	...
	/**
	 * Modify the receipt output for composited items.
	 *
	 * @since 10.3.4
	 ...
	 */
	public function add_composited_item_order_receipt_data( array $line_item_display_data, array $line_item_data ): array {

		if ( ! isset( $line_item_data['item'] ) ) {
			return $line_item_display_data;
		}

		if ( wc_cp_is_composited_order_item( $line_item_data['item'] ) ) {

			$line_item_display_data['tr_attributes']['class'] = trim( ( $line_item_display_data['tr_attributes']['class'] ?? '' ) . ' composited_table_item' );
			...
			$container_order_item = wc_cp_get_composited_order_item_container( $line_item_data['item'] );

			if ( ! is_a( $container_order_item, 'WC_Order_Item' ) ) {
				return $line_item_display_data;
			}

			$composite_data = $container_order_item->get_data();

			if ( ! isset( $composite_data['name'] ) || ! isset( $line_item_data['title'] ) ) {
				return $line_item_display_data;
			}

			$aria_label = sprintf(
			/* translators: %1$s: Composited item title, %2$s: Composite title */
				__( '%1$s (Part of: %2$s)', 'woocommerce-composite-products' ),
	...
	/**
	 * Style the indent arrow for composited items products.
	 *
	 * @since 10.3.4
	 * @param string $css The style rules for the receipt engine.
	 * @return string The updated css rules.
	 */
	public function add_composited_item_order_receipt_styles( string $css ): string {
		$css .= '
		.composited_table_item .indent-arrow { display: inline-block; transform: scaleX(-1); padding-right: 1em; padding-left: .25em; }
		';
		return $css;
	}
}
