...
class WC_CP_Cart {
	...
	*/
	...
	/**
	 * Map JSON from Store API request to composite configuration used in the rest of the plugin.
	 *
	 * Roughly the following mapping:
	 *
	 * "composite_configuration": [
	 *  {
	 *    "component_id": "1732525973", // First component_id
	 *    "product_id": 607,            // wccp_component_selection[component_id]
	 *    "quantity": 1,                // wccp_component_quantity[component_id]
	 *    "use_default_options": true,  // "action": "woocommerce_add_order_item"
	 *  },
	 *  {
	 *    "component_id": "1732525974", // Second component id
	 *    "product_id": 578,            // wccp_component_selection[component_id]
	 *    "quantity": 2,                // wccp_component_quantity[component_id]
	 *    "variation_id": 580,          // wccp_variation_id[component_id]
	 *    "attributes": [
	 *      {
	 *        "name": "color",
	 *        "option": "blue"          // wccp_attribute_color[component_id]
	 *      }
	 *    ]
	 *  }
	 * ]
	 *
	 * When a component is hidden by a scenario on the front end, POSTed data also contains
	 * `wccp_component_selection_nil[component_id]`, but the information is carried in
	 * the qty = 0 already. _nil only means that component_id will become an empty string
	 * during parsing, so is not needed further on.
	 *
	 * Since WC_CP_Cart::configure_cart_data validates taxonomies etc, this method only maps data.
	 *
	 * @since 10.3.0
	 *
	 ...
	 * @return array
	 */
	protected static function map_store_api_composite_configuration( $store_api_configuration ) {
		$composite_configuration = array();

		foreach ( $store_api_configuration as $component ) {
			// Component id is compulsory.
			$component_id = $component['component_id'];

			$composite_configuration['wccp_component_selection'][ $component_id ] = $component['product_id'] ?? '';
			$composite_configuration['wccp_component_quantity'][ $component_id ]  = $component['quantity'] ?? null;

			if ( isset( $component['variation_id'] ) ) {
				$composite_configuration['wccp_variation_id'][ $component_id ] = $component['variation_id'];
			}

			if ( isset( $component['attributes'] ) && is_array( $component['attributes'] ) ) {
				foreach ( $component['attributes'] as $attribute ) {
					$composite_configuration[ 'wccp_attribute_' . $attribute['name'] ][ $component_id ] = $attribute['option'];
				}
			}
		}

		// Allow adding static components to order without needing to explicitly set product id.
		$composite_configuration['action'] = 'woocommerce_add_order_item';

		return $composite_configuration;
	}

	/**
	 * Adds Composite product's ['composite_data'] data to ['cart_item_data'] based on a Store API request.
	 *
	 ...
	 * @return array|mixed
	 */
	public function handle_store_api_add_to_cart_request( $add_to_cart_data, $wp_rest_request ) {
		$params = $wp_rest_request->get_json_params();

		$product_id = $add_to_cart_data['id'];

		if ( ! isset( $params['composite_configuration'] ) ) {
		...
		$store_api_cart_data = $this->configure_cart_data( self::map_store_api_composite_configuration( $params['composite_configuration'] ), $product_id );

		/**
		 * 'woocommerce_store_api_composite_configuration' filter.
		 *
		 * @since 10.3.0
		 *
		 ...
		 * @param WP_REST_Request      $wp_rest_request     Store API's request object.
		 */
		$add_to_cart_data['cart_item_data']['composite_data'] = apply_filters( 'woocommerce_store_api_composite_configuration', $store_api_cart_data, $add_to_cart_data, $wp_rest_request );

		return $add_to_cart_data;
	}

	/**
	 * Create composite configuration based on input data (either from POST/GET request or Store API).
	 *
	 * $input_config is a one dimensional array with keys like:
	 *  - wccp_component_selection[ID]
	 *  - wccp_component_quantity[ID]
	 *  - wccp_variation_id[ID]
	 *  - wccp_attribute_XYZ[ID]
	 *
	 * ...where
	 *  - ID is the component item's ID
	 *
	 * @since 10.3.0
	 *
	 * @param array                    $input_config Array describing the configuration of the bundle.
	 * @param WC_Product_Composite|int $composite Composite product object.
	 *
	 * @return array
	 */
	protected function configure_cart_data( $input_config, $composite ) {
		$posted_config = array();
		...
		foreach ( $components as $component_id => $component ) {
			...
			$composited_product_id                = isset( $input_config['wccp_component_selection'] ) && ! empty( $input_config['wccp_component_selection'][ $component_id ] ) ? absint( $input_config['wccp_component_selection'][ $component_id ] ) : '';
			$composited_product_quantity          = isset( $input_config['wccp_component_quantity'], $input_config['wccp_component_quantity'][ $component_id ] ) ? absint( $input_config['wccp_component_quantity'][ $component_id ] ) : $component->get_quantity( 'min' );
			$composited_product_sold_individually = false;

			if ( ! $composited_product_id && $component->is_static() ) {
				if ( ! empty( $input_config['action'] ) && 'woocommerce_add_order_item' === $input_config['action'] ) {
					$composited_product_id = $component->get_default_option();
				}
			}

			$composited_product_id = isset( $input_config['wccp_component_selection_nil'], $input_config['wccp_component_selection_nil'][ $component_id ] ) ? '' : $composited_product_id;
			...
			if ( 'variable' === $composited_product_type ) {
				...
				$variation   = empty( $input_config['wccp_variation_id'][ $component_id ] ) ? '' : absint( wp_unslash( $input_config['wccp_variation_id'][ $component_id ] ) );
				$attr_config = array();
				...
				foreach ( $attributes as $attribute ) {
					...
					if ( isset( $input_config[ 'wccp_' . $taxonomy ][ $component_id ] ) && '' !== $input_config[ 'wccp_' . $taxonomy ][ $component_id ] ) {

						// Get value from post data.
						if ( $attribute->is_taxonomy() ) {
							$value = sanitize_title( stripslashes( $input_config[ 'wccp_' . $taxonomy ][ $component_id ] ) );
						} else {
							$value = html_entity_decode( wc_clean( stripslashes( $input_config[ 'wccp_' . $taxonomy ][ $component_id ] ) ), ENT_QUOTES, get_bloginfo( 'charset' ) );
						}
	 ...
	 *    );
	 *
	 * @param  mixed $composite Composite product object or ID.
	 * @return array|mixed
	 */
	public function get_posted_composite_configuration( $composite ) {
		...
		if ( is_object( $composite ) && $composite->is_type( 'composite' ) ) {
			...
			$posted_config = $this->configure_cart_data( $posted_data, $composite );
	 ...
	 *
	 * @return void
	 * @throws \Automattic\WooCommerce\StoreApi\Exceptions\RouteException When add to cart isn't valid.
	 */
	public function validate_add_to_cart_store_api( $product, $request ) {

		if ( ! $product->is_type( 'composite' ) ) {
			return;
		}

		$composite_data = $request['cart_item_data']['composite_data'] ?? array();
		$quantity       = $request['quantity'] ?? 1;

		try {
			$this->validate_composite_configuration( $product, $quantity, $composite_data );
		} catch ( Exception $e ) {
			throw new \Automattic\WooCommerce\StoreApi\Exceptions\RouteException( 'woocommerce_rest_cart_invalid_composite_product', esc_html( $e->getMessage() ) );
		}
	}
