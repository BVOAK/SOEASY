jQuery(document).ready(function ($) {
  ...
  window.initStep2Events = function () {
    ...
    $(document).on('change', '.forfait-internet-checkbox', function () {
      ...
      if ($clicked.is(':checked')) {
        ...
        saveToLocalConfig(index, 'abonnements', [], { replace: true });
        saveToLocalConfig(index, 'materiels', [], { replace: true });
  ...
  window.initStep4Events = function () {
    ...
    function saveCentrexQuantites(index) {
      ...
      const equipementsIds = [];
      let totalPostesCentrex = 0;
      ...
      // Sauvegarde dans localStorage (abonnements / materiels)
      Object.entries(produitsParSection).forEach(([section, produits]) => {
        const types = new Set(produits.map(p => p.type));
        types.forEach(type => {
          const produitsFiltres = produits.filter(p => p.type === type);
      ...
      // Frais d'installation Centrex (postes + licences)
      const tousFrais = window.fraisInstallationCentrexParProduit || {};
      const fraisSelectionnes = [];
      const config = JSON.parse(localStorage.getItem('soeasyConfig')) || {};
      config[index] = config[index] || {};

      // — 1. Frais liés aux équipements (par nombre de postes)
        ...
        if (Array.isArray(tousFrais[id])) {
          tousFrais[id].forEach(f => {
            ...
            if (postesCentrex >= min && postesCentrex <= max) {
              fraisSelectionnes.push({ ...f, type: 'centrex', quantite: 1 });
            }
          });
        }
      });

      // — 2. Frais liés aux licences Centrex (si en-dessous du seuil)
      const licencesCentrex = sessionPayload.licences_centrex.reduce((sum, item) => sum + item.qty, 0);
      const fraisLicenceAjoutes = new Set();

      sessionPayload.licences_centrex.forEach(({ id }) => {
        const fraisAssocies = tousFrais[id] || [];
        fraisAssocies.forEach(f => {
          const seuil = parseInt(f.offertAPartirDe) || 9999;
          const isFraisLicence = f.nom?.toLowerCase().includes('licence centrex');

          if (isFraisLicence && licencesCentrex < seuil && !fraisLicenceAjoutes.has(f.id)) {
            fraisLicenceAjoutes.add(f.id);
            fraisSelectionnes.push({ ...f, type: 'centrex', quantite: 1 });
          }
        });
      });

      // — 3. Injection dans le localStorage
      const existants = config[index].fraisInstallation || [];
      const autres = existants.filter(f => !f.type || f.type !== 'centrex');

      config[index].fraisInstallation = [...autres, ...fraisSelectionnes];
      localStorage.setItem('soeasyConfig', JSON.stringify(config));

      $.post(soeasyVars.ajaxurl, {
        action: 'soeasy_set_frais_installation',
        index,
        items: fraisSelectionnes
      });
  ...
  window.initStep5Events = function () {
    ...
    function updateFraisTotal(index) {
      ...
      const formatted = new Intl.NumberFormat('fr-FR', {
        style: 'currency',
        currency: 'EUR'
      }).format(safeTotal) + (mode === 'leasing' ? ' / mois' : '');
    ...
    $('.frais-installation-list').each(function () {
      ...
      if (frais && Array.isArray(frais) && frais.length > 0) {
        frais.forEach(item => {
          $list.find(`.frais-checkbox[data-id="${item.id}"]`).prop('checked', true);
        });
      } else if (frais && typeof frais === 'object' && frais.reporte === true) {
