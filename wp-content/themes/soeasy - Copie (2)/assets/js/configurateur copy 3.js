    ...
    function updateFraisInstallationFromInternet(index) {
      ...
      equipementsIds.forEach(id => {
        if (Array.isArray(tousFrais[id])) {
          tousFrais[id].forEach(f => fraisSelectionnes.push(f));
        }
      });

      config[index].fraisInstallation = fraisSelectionnes.map(f => ({ ...f, type: 'internet' }));
      ...
      equipementsIds.forEach(id => {
        if (Array.isArray(tousFrais[id])) {
          tousFrais[id].forEach(f => {
            const clone = { ...f, type: 'centrex' };
            fraisSelectionnes.push(clone);
          });
        }
      });
